...
Route::prefix('api/banking')->middleware(['api', 'auth.session'])->group(function () {
    // Dashboard (read only)
    Route::get('dashboard', [BankingController::class, 'dashboard']);

    // Banking operations with model access middleware
    Route::middleware(['model.access:banking.bankstatement'])->group(function () {
        // Bank Accounts
...
// Public webhook endpoint (no auth.session)
