...
Route::prefix('api/cashbook')->middleware(['api', 'auth.session'])->group(function () {
    // Stats and summary (read permission via controller)
    Route::get('/stats', [CashBookController::class, 'stats']);
    Route::get('/summary', [CashBookController::class, 'summary']);
    
    // Open invoices for allocation (read)
    Route::get('/open-invoices', [CashBookController::class, 'getOpenInvoices']);
    
    // Entry operations with model access middleware
    Route::middleware(['model.access:cashbook.entry'])->group(function () {
        // Transfers
